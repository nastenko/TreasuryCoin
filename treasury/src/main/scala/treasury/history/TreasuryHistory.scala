...
import treasury.validation.{TDifficultyBlockValidator, TParentBlockValidator, TSemanticBlockValidator}
...
/**
  ...
  */
class TreasuryHistory(val storage: THistoryStorage,
                     settings: HybridMiningSettings,
                     validators: Seq[BlockValidator[HybridBlock]],
                     statsLogger: Option[FileLogger])
  extends History[HybridBlock, HybridSyncInfo, TreasuryHistory] with ScorexLogging {

  import TreasuryHistory._

  override type NVCT = TreasuryHistory
  ...
  private def powBlockAppend(powBlock: PowBlock): (TreasuryHistory, ProgressInfo[HybridBlock]) = {
    ...
    (new TreasuryHistory(storage, settings, validators, statsLogger), progress)
  }

  private def posBlockAppend(posBlock: TPosBlock): (TreasuryHistory, ProgressInfo[HybridBlock]) = {
    ...
    (new TreasuryHistory(storage, settings, validators, statsLogger), mod)
  }
  ...
  /**
    ...
    * @return
    */
  override def append(block: HybridBlock): Try[(TreasuryHistory, ProgressInfo[HybridBlock])] = Try {
    ...
    val res: (TreasuryHistory, ProgressInfo[HybridBlock]) = block match {
      case powBlock: PowBlock => powBlockAppend(powBlock)
      case posBlock: TPosBlock => posBlockAppend(posBlock)
    }
  ...
  private def calcDifficultiesForNewBlock(posBlock: TPosBlock): (BigInt, BigInt) = {
  ...
  def blockGenerator(m: HybridBlock): PublicKey25519Proposition = m match {
    case p: TPosBlock => p.generatorBox.proposition
  ...
  def parentBlock(m: HybridBlock): Option[HybridBlock] = m match {
    case b: TPosBlock => modifierById(b.parentId)
  ...
  private def chainBack(m: HybridBlock,
    ...
    val sum: Seq[(ModifierTypeId, ModifierId)] = if (m.isInstanceOf[TPosBlock]) (TPosBlock.ModifierTypeId -> m.id) +: acc
  ...
  override def reportSemanticValidity(modifier: HybridBlock,
                                      valid: Boolean,
                                      lastApplied: ModifierId): (TreasuryHistory, ProgressInfo[HybridBlock]) = {
...
object TreasuryHistory extends ScorexLogging {
  val DifficultyRecalcPeriod = 20

  def readOrGenerate(settings: ScorexSettings, minerSettings: HybridMiningSettings): TreasuryHistory = {
    readOrGenerate(settings.dataDir, settings.logDir, minerSettings)
  }

  def readOrGenerate(dataDir: File, logDir: File, settings: HybridMiningSettings): TreasuryHistory = {
    ...
    val storage = new THistoryStorage(blockStorage, settings)
    val validators = Seq(new TDifficultyBlockValidator(settings, storage),
      new TParentBlockValidator(storage),
      new TSemanticBlockValidator(Blake2b256)
    )

    new TreasuryHistory(storage, settings, validators, Some(logger))
  }
}
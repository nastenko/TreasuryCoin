...
class THistoryStorage(storage: LSMStore,
  ...
  def bestPosBlock: TPosBlock = {
    require(height > 0, "History is empty")
    modifierById(bestPosId).get.asInstanceOf[TPosBlock]
  }
  ...
  def modifierById(blockId: ModifierId): Option[HybridBlock with
    ...
    storage.get(ByteArrayWrapper(blockId)).flatMap { bw =>
      ...
      val parsed = mtypeId match {
        ...
        case t: Byte if t == TPosBlock.ModifierTypeId =>
          TPosBlockCompanion.parseBytes(bytes.tail)
      }
      parsed match {
  ...
  def update(b: HybridBlock, diff: Option[(BigInt, BigInt)], isBest: Boolean) {
    ...
    val bestBlockSeq: Iterable[(ByteArrayWrapper, ByteArrayWrapper)] = b match {
      ...
      case posBlock: TPosBlock if isBest =>
  ...
  def parentId(block: HybridBlock): ModifierId = block match {
    ...
    case posBlock: TPosBlock => posBlock.parentId
  }
  ...
  def isGenesis(b: HybridBlock): Boolean = b match {
    ...
    case posB: TPosBlock => heightOf(posB.parentId).contains(1L)
  }
}
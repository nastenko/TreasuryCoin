...
case class TBoxStoredState(store: LSMStore, override val version: VersionTag) extends
  BoxMinimalState[PublicKey25519Proposition,
    PublicKey25519NoncedBox,
    SimpleBoxTransaction,
    HybridBlock,
    TBoxStoredState] with ScorexLogging {
  ...
  override type NVCT = TBoxStoredState
  type HPMOD = HybridBlock

  override def semanticValidity(tx: SimpleBoxTransaction): Try[Unit] = TBoxStoredState.semanticValidity(tx)
  ...
  override def applyChanges(changes: BoxStateChanges[PublicKey25519Proposition, PublicKey25519NoncedBox],
                            newVersion: VersionTag): Try[TBoxStoredState] = Try {
    ...
    log.trace(s"Update TBoxStoredState from version $lastVersionString to version ${Base58.encode(newVersion)}. " +
    ...
    val newSt = TBoxStoredState(store, newVersion)
  ...
  override def rollbackTo(version: VersionTag): Try[TBoxStoredState] = Try {
    if (store.lastVersionID.exists(_.data sameElements version)) {
      this
    } else {
      log.info(s"Rollback TBoxStoredState to ${Base58.encode(version)} from version $lastVersionString")
      store.rollback(ByteArrayWrapper(version))
      new TBoxStoredState(store, version)
    }
  }.recoverWith{case e =>
    log.error("Cant' do rollback: ", e)
    Failure[TBoxStoredState](e): Try[TBoxStoredState]
  }
...
object TBoxStoredState {
  ...
  def readOrGenerate(settings: ScorexSettings): TBoxStoredState = {
    ...
    TBoxStoredState(stateStorage, version)
  }

  def genesisState(settings: ScorexSettings, initialBlocks: Seq[HybridBlock]): TBoxStoredState = {

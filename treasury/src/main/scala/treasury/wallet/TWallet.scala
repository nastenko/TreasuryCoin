...
case class TWallet(seed: ByteStr, store: LSMStore)
  extends Wallet[PublicKey25519Proposition, SimpleBoxTransaction, HybridBlock, TWallet]
    with ScorexLogging {
  ...
  override def generateNewSecret(): TWallet = {
  ...
  override def scanOffchain(tx: SimpleBoxTransaction): TWallet = this

  override def scanOffchain(txs: Seq[SimpleBoxTransaction]): TWallet = this

  override def scanPersistent(modifier: HybridBlock): TWallet = {
    log.debug(s"Applying modifier to wallet: ${Base58.encode(modifier.id)}")
    val changes = TBoxStoredState.changes(modifier).get
  ...
  override def rollback(to: VersionTag): Try[TWallet] = Try {
...
object TWallet {
  ...
  def readOrGenerate(settings: ScorexSettings, seed: ByteStr): TWallet = {
    ...
    TWallet(seed, boxesStorage)
  }

  def readOrGenerate(settings: ScorexSettings): TWallet = {
    readOrGenerate(settings, settings.wallet.seed)
  }

  def readOrGenerate(settings: ScorexSettings, seed: ByteStr, accounts: Int): TWallet =
    (1 to accounts).foldLeft(readOrGenerate(settings, seed)) { case (w, _) =>
      w.generateNewSecret()
    }

  def readOrGenerate(settings: ScorexSettings, accounts: Int): TWallet =
    (1 to accounts).foldLeft(readOrGenerate(settings)) { case (w, _) =>
      w.generateNewSecret()
    }

  //wallet with applied initialBlocks
  def genesisWallet(settings: ScorexSettings, initialBlocks: Seq[HybridBlock]): TWallet = {

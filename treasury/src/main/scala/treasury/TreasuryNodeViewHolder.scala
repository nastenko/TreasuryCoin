...
import treasury.block.{TPosBlock, TPosBlockCompanion}
import treasury.history.TreasuryHistory
import treasury.state.TBoxStoredState


class TreasuryNodeViewHolder(settings: ScorexSettings, minerSettings: HybridMiningSettings)
  ...
  override type HIS = TreasuryHistory
  override type MS = TBoxStoredState
  ...
  override val modifierSerializers: Map[ModifierTypeId, Serializer[_ <: NodeViewModifier]] =
    Map(TPosBlock.ModifierTypeId -> TPosBlockCompanion,
  ...
  override protected def genesisState: (HIS, MS, VL, MP) = {
    ...
    val posGenesis = TPosBlock.create(powGenesis.id, 0, genesisTxs, genesisBox, attachment, genesisAccountPriv)

    var history = TreasuryHistory.readOrGenerate(settings, minerSettings)
    history = history.append(powGenesis).get._1
    history = history.append(posGenesis).get._1

    val gs = TBoxStoredState.genesisState(settings, Seq(posGenesis, powGenesis))
  ...
  override def restoreState(): Option[(HIS, MS, VL, MP)] = {
    if (HWallet.exists(settings)) {
      Some((
        TreasuryHistory.readOrGenerate(settings, minerSettings),
        TBoxStoredState.readOrGenerate(settings),

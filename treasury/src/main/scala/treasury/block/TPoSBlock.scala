...
case class TPosBlock(override val parentId: BlockId, //PoW block
  ...
  override type M = TPosBlock

  override lazy val serializer = TPosBlockCompanion

  override lazy val version: Version = 0: Byte

  override lazy val modifierTypeId: ModifierTypeId = TPosBlock.ModifierTypeId
  ...
  override def toString: String = s"TPoSBlock(${json.noSpaces})"
}

object TPosBlockCompanion extends Serializer[TPosBlock] {
  override def toBytes(b: TPosBlock): Array[Byte] = {
  ...
  override def parseBytes(bytes: Array[Byte]): Try[TPosBlock] = Try {
    require(bytes.length <= TPosBlock.MaxBlockSize)
    ...
    TPosBlock(parentId, timestamp, txs, box, attachment, signature)
  }
}
